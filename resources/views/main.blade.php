        ...
        @auth
            <div class="row">
                <div class="col">
                    ...
                    @if ($tasks->isEmpty())
                        <!-- <h1>My Tasks</h1> <p>No tasks found.</p> -->
                            ...
                            <tbody>
                                @foreach ($tasks as $task)
                                    <tr class="{{ $task->priority ? 'high-priority' : '' }}">
                                        ...
                                        <td>{{ $task->due_date instanceof \Carbon\Carbon ? $task->due_date->format('Y-m-d') : \Carbon\Carbon::parse($task->due_date)->format('Y-m-d') }}</td>
                                        <td>
                                        ...
                                        <td>
                                            @if (!$task->completed)
                                                <form action="{{ route('tasks.complete', $task->id) }}" method="POST">
                                                    @csrf
                                                    <button type="submit" class="btn btn-success" data-task-id="{{ $task->id }}">Complete</button>
                                                </form>
                                            @endif
                                        </td>
                                        <td>
